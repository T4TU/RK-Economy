...
public class MarketListener implements Listener {
	...
	public void onAsyncPlayerChat(AsyncPlayerChatEvent e) {
		...
		if (name.containsKey(player)) {
			...
			try {
				...
				if (message.length() <= 20) {
					...
					player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_NO, 1, 1);
					player.sendMessage(tc3 + "Antamasi nimi on liian pitkä. Nimen maksimipituus on 20 merkkiä. (Huomiothan, että liian pitkät nimet eivät näy kylteissä oikein.)");
					name.remove(player);
				}
			}
			catch (Exception ex) {
			...
			try {
				...
				if (amount > 0) {
					...
					if (amount <= amountInStorage) {
						...
						double endPrice = amount * (sellingPoint.getPrice() / sellingPoint.getAmount());
						if (CoreUtils.hasEnoughRoom(player, item, amount, economy.SILVER_COIN, 9)) {
							if (economy.takeCash(player, endPrice)) {
								...
								player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1, 2);
								player.sendMessage(tc2 + "Ostit " + tc1 + amount + tc2 + " kappaletta tuotetta " + tc1 + itemName + tc2 + " hintaan " + tc1 + endPrice + "£" + tc2 + "!");
								...
								String logEntry = "[" + f.format(date) + "] " + player.getName() + ": " + amount + "kpl " + ChatColor.stripColor(itemName) + " (+" + endPrice + "£)";
		...
		if (price.containsKey(player)) {
			...
			try {
				...
				if (price > 0) {
					player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1, 2);
					player.sendMessage(tc2 + "Asetettiin myyntipisteen hinnaksi " + tc1 + price + "£" + tc2 + "!");
	...
	public void onInventoryClick(InventoryClickEvent e) {
		...
		if (e.getInventory() != null && e.getInventory().getTitle() != null && e.getInventory().getTitle().equals("Tuotteen esikatselu")) {
			e.setCancelled(true);
		}
			...
			if (CoreUtils.getDisplayName(e.getCurrentItem()).equals("§a§lHinnan tyyppi")) {
				...
				try {
					...
					if (lore.get(0).startsWith("§a")) {
						inventory.setItem(25, CoreUtils.getItem(Material.SIGN, "§a§lHinnan tyyppi", Arrays.asList("§7  £/1kpl", "§a> £/10kpl <", "§7  £/64kpl"), 1));
						sellingPoint.setAmount(10);
					}
					else if (lore.get(1).startsWith("§a")) {
						inventory.setItem(25, CoreUtils.getItem(Material.SIGN, "§a§lHinnan tyyppi", Arrays.asList("§7  £/1kpl", "§7  £/10kpl", "§a> £/64kpl <"), 1));
						sellingPoint.setAmount(64);
					}
					else {
						inventory.setItem(25, CoreUtils.getItem(Material.SIGN, "§a§lHinnan tyyppi", Arrays.asList("§a> £/1kpl <", "§7  £/10kpl", "§7  £/64kpl"), 1));
					...
					if (marketStall.getOwnerUuid().equals(player.getUniqueId().toString()) && player.isSneaking()) {
						...
						new BukkitRunnable() {
							public void run() {
								...
								double price = sellingPoint.getPrice();
								...
								player.sendMessage(tc2 + " Hinta: " + tc1 + price + "£/" + amount + "kpl");
			...
			if (frame != null) {
				...
				for (int i = 0; i < sellingPoint.getInventory().getContents().length - 2; i++) {
					...
					if (CoreUtils.isNotAir(item)) {
						...
						meta.setDisplayName("§a" + LanguageHelper.getItemDisplayName(preview, "fi_FI") + "§6 " + sellingPoint.getPrice() + "£/" + sellingPoint.getAmount() + "kpl");
